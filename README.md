<div align="center">

# 김학태 (Hak-tae Kim)
### Embedded Systems / IoT Developer

📍 Busan, South Korea · 🎓 PKNU IoT Developer Track

[![GitHub](https://img.shields.io/badge/GitHub-kimht4040-181717?style=flat-square&logo=github)](https://github.com/kimht4040)
&nbsp;
<!-- 아래 이메일/링크드인은 본인 정보로 교체하세요 -->
[![Email](https://img.shields.io/badge/Email-your.email%40example.com-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:your.email@example.com)

</div>

---

## About

임베디드/IoT 시스템 개발에 관심을 두고 있는 개발자입니다. 실시간 통신, 제어 시스템, 센서-액추에이터 연동을 주제로 한 프로젝트들을 진행하며 C/C++ 기반의 시스템 프로그래밍 역량을 쌓아왔습니다.

- **Languages:** C, C++, Python, C#
- **Embedded / OS:** Yocto Linux, 임베디드 리눅스 환경 빌드 및 배포
- **통신/프로토콜:** TCP/UDP 소켓 통신, ALSA (오디오 I/O)
- **기타:** .NET, Unity(시뮬레이션/시각화), SQL

---

## Featured Projects

### 🔒 Tactical Voice Scrambler
**Yocto 기반 전술 통신용 실시간 음성 비화 및 C2 모니터링 시스템**
`C++` `UDP/TCP` `ALSA` `Yocto Linux`

임베디드 리눅스(Yocto) 환경에서 동작하는 실시간 음성 암호화(비화) 및 지휘통제(C2) 모니터링 시스템. 네트워크 소켓 통신과 ALSA를 이용한 저지연 오디오 처리를 직접 구현.

🔗 [github.com/kimht4040/tactical-voice-scrambler](https://github.com/kimht4040/tactical-voice-scrambler)

### 🎯 Target Tracking System
**타겟을 추적하는 터렛(turret) 제작 프로젝트**
`C` `제어 시스템` `센서 연동`

객체를 실시간으로 추적하여 물리적으로 조준하는 터렛 시스템. 센서 입력 처리와 모터 제어 로직을 설계 및 구현.

🔗 [github.com/kimht4040/target_tracking_system](https://github.com/kimht4040/target_tracking_system)

### 🦾 Conveyor Sorting Robot Arm (Team Project)
**컨베이어벨트 위 물체를 분류하여 로봇팔로 처리하는 팀 프로젝트**
`C` `로봇팔 제어` `팀 프로젝트`

컨베이어벨트로 이동하는 물체를 감지·분류하고, 로봇팔이 이를 처리하도록 연동한 프로젝트. 팀 단위 협업 및 하드웨어-소프트웨어 통합 경험.

🔗 [github.com/kimht4040/con](https://github.com/kimht4040/con)

### 🔌 IoT .NET Project
**IoT 개발자 과정 .NET 기반 리포지토리**
`.NET` `C#` `Unity`

.NET 환경에서의 IoT 애플리케이션 개발 및 Unity를 활용한 시각화/시뮬레이션 작업.

🔗 [github.com/kimht4040/iot-dotnet-2026](https://github.com/kimht4040/iot-dotnet-2026)

---

## Coursework & Fundamentals

IoT 개발자 과정에서 이수한 기초 역량 강화 리포지토리입니다.

| Repository | 내용 |
|---|---|
| [iot-cpp](https://github.com/kimht4040/iot-cpp) | C++ 학습 |
| [iot-algorithm-2026](https://github.com/kimht4040/iot-algorithm-2026) | 알고리즘 학습 |
| [iot-database-2026](https://github.com/kimht4040/iot-database-2026) | 데이터베이스 |
| [iot-python-2026](https://github.com/kimht4040/iot-python-2026) | 파이썬 학습 |
| [iot-basic-programming](https://github.com/kimht4040/iot-basic-programming) | 기초 프로그래밍 |

<details>
<summary>기타 사이드 프로젝트</summary>
<br>

- **[OsuRhythmGame](https://github.com/kimht4040/OsuRhythmGame)** — osu 파일을 파싱하여 구현한 리듬게임 (C++)

</details>

---

## GitHub Stats

<div align="center">
<img height="160" src="https://github-readme-stats.vercel.app/api?username=kimht4040&show_icons=true&theme=default&hide_border=true&count_private=true" />
<img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=kimht4040&layout=compact&theme=default&hide_border=true" />
</div>
